...
final class WC_Dintero_HP {
	...
	public function template_redirect() {
		...
		if ( $consentremoval ) {
			return $this->dintero_consent_removal_callback( $consentremoval );
		}
	...
	private function is_consent_removal() {
		if ( sanitize_text_field( wp_unslash( $_SERVER['REQUEST_METHOD'] ) ) != 'DELETE' ) {
			return false;
		}
		if ( ! get_option( 'permalink_structure' ) ) {
	...
	public function is_special_page() {
		$specials = array(
			'dintero-betaling'         => 'dintero_wait_for_payment',
			'dintero-express-checkout' => 'dintero_express_checkout',
			'dintero-buy-product'      => 'dintero_buy_product',
		);
		$method   = null;
		if ( get_option( 'permalink_structure' ) ) {
