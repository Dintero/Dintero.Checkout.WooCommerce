...
class WC_AJAX_CH {
	...
	public static function get_endpoint( $request = '' ) {
		return esc_url_raw( apply_filters( 'woocommerce_ajax_get_endpoint', add_query_arg( 'dch-ajax', $request, remove_query_arg( array( 'remove_item', 'add-to-cart', 'added-to-cart', 'order_again', '_wpnonce' ), home_url( '/', 'relative' ) ) ), $request ) );
	}
	...
	public static function do_wc_ajax() {
		...
		if ( ! empty( $_GET['dch-ajax'] ) ) {
			$wp_query->set( 'dch-ajax', sanitize_text_field( wp_unslash( $_GET['dch-ajax'] ) ) );
		}

		$action = $wp_query->get( 'dch-ajax' );
		
		if ( $action ) {
			self::wc_ajax_headers();
			$action = sanitize_text_field( $action );
			do_action( 'dch_ajax_' . $action );
			wp_die();
		}
		// phpcs:enable
	}
	...
	public static function add_ajax_events() {
		...
		foreach ( $ajax_events_nopriv as $ajax_event ) {
			...
			add_action( 'dch_ajax_' . $ajax_event, array( __CLASS__, $ajax_event ) );
		}
	}
	...
	public function update_session(){
		...
		$session = WCDCH()->checkout()->update_session($sessionId);
		$result = array(
			'success' => true
		);
		wp_send_json($result);
	...
	*
	*/
	public function dch_create_order(){
		...
		if ( ! empty( $_GET['transaction_id'] ) ) {
			...
			if(!$order && $transaction['merchant_reference_2'] == ''){
				...
				$payment_method     = $available_gateways['dintero-checkout'];
				...
				$transaction = WCDCH()->checkout()->update_transaction($transaction_id, $order->get_id());
	...
	*/
	...
	public static function express_pay() {
		...
		$test = WCDCH()->checkout()->pay_action(true);
	}
	...
	*/
	...
	public static function dch_update_ord_emded(){
		
	}
	/**
	 * Update order status post back
	 */
	public static function dch_update_ord() {
		if ( ! empty( $_GET['transaction_id'] ) ) {
	 ...
	 */
	public static function dch_update_ship() {
	...
	public static function payment_complete( $order, $transaction_id = '', $note = '' ) {
		...
		WCDCH()->checkout()->create_receipt( $order );
	}
	...
	private static function process_authorization( $order, $transaction_id = '', $reason = '' ) {
		...
		$default_order_status = WC_Dintero_CH_Admin_Settings::get_option('default_order_status');
