...
class WC_Dintero_CH_Checkout extends WC_Checkout {

	/**
	 * The single instance of the class.
	 * 
	 * @var WC_Dintero_CH_Checkout|null
	 */
	protected static $instance = null;	

	private $id = 'dintero-checkout';
	private $enabled;
	private $test_mode;
	private $payment_method = 'dintero-checkout';
	private $account_id;
	private $client_id;
	...
	/**
	 * Gets the main WC_Dintero_CH_Checkout Instance.
	 *
	 * @since 2.1
	 * @static
	 * @return WC_Dintero_CH_Checkout Main instance
	 */
		...
		if ( true == $pay_for_order || ( false == $pay_for_order && !WC()->cart->is_empty() ) ) {
			...
			if ( !$express && $embed_enable != 'yes') {
				...
				if ( true == $pay_for_order ) {
					$this->validate_pay_ch( $posted_data, $errors );
				} else {
			...
			if ( 0 === wc_notice_count( 'error' ) ) {
				if ( true == $pay_for_order ) {
					...
					if ( empty( $posted_data['woocommerce_checkout_update_totals'] ) ) {
						...
						if ( !$express ) {
							$order_id = $this->create_order_ch( $posted_data );
						}
					}
				}
				...
				if(WCDCH()->setting()->get('embed_enable') == 'yes'){
	...
	public function get_icon( $type = '', $width = '', $img = true ) {
		if ($this->profile_id) {
			$logo_w = WCDCH()->setting()->get('checkout_logo_width');

			if ( !$width ) {
				$width = WCDCH()->setting()->get('checkout_logo_width');
				if ( !$width ) {
					$width = WCDCH()->setting()->getDefault('checkout_logo_width');
				}
			}
	...
	public function get_icon_footer( $width = '' ) {
		$icon_url = trim(WCDCH()->setting()->get('branding_footer_url'));
		if ('' == $icon_url) {
	...
	public function get_icon_checkout( $width = '' ) {
		$icon_url = trim(WCDCH()->setting()->get('branding_checkout_url'));
		if ('' == $icon_url) {
	 ...
	 */
	private function create_order_ch( $data ) {
	 ...
	 * Display error notice on invalid
	 */
	private function validate_pay_ch( &$data, &$errors ) {
		try {
			$this->validate_posted_data( $data, $errors );
			$this->check_cart_items();
	...
	private function get_access_token() {
		...
		$headers = array(
			...
			'Dintero-System-Plugin-Version' => DINTERO_CH_VERSION
		);
	...
	private function process_payment_embed_express ($express = true, $pay_for_order = false ){
		...
		$chossedShippignMethod = WC()->session->get( 'chosen_shipping_methods' )[0];

		if(!$chossedShippignMethod){
			...
			if(!$postCode){	 
				...
				echo( "<script type=\"text/javascript\">
						...
						function savePostCode(){
							...
							var url = \"".home_url().'?dch-ajax=update_shipping_postcode'."\";
							var data = {
		...
		if (1 == $result && $url) {
			...
			echo( "<script type=\"text/javascript\">
					var dintero_url = \"".home_url().'?dch-ajax=update_session'."\";
					    ...
					    if(typeof(dintero) != \"undefined\"){
					        ...
					        .embed({
							    ...
							    onSessionLocked: (event, checkout) => {
				            		...
				            		var url = \"".home_url().'?dch-ajax=update_session'."\";
	...
	private function process_payment_embed( $order_id, $express = false, $pay_for_order = false ) {
		...
		if ( ! empty( $order ) && $order instanceof WC_Order ) {
			...
			if (1 == $result && $url) {
				...
				//echo( '<div class="dhp-logo">' . wp_kses_post( WCDCH()->checkout()->get_icon_checkout() ) . '</div>' );
				echo( '<div id="dintero-checkout-iframe"></div>' );
				echo( '</div>' );
	...
	private function get_iframe(){
		...
		//$callback_url = home_url() . '?dch-ajax=dch_update_ord_emded';
		$callback_url = home_url() . '?dch-ajax=dch_create_order&delay_callback=180';
		$cart = WC()->cart;
		...
		if ( WC()->shipping->get_packages() && WC()->session->get( 'chosen_shipping_methods' )[0] )  {
			$ship_callback_url = home_url() . '?dch-ajax=dch_update_ship';
			$selectedShippingReference = $this->get_shipping_reference();
			$express_option = array(
	...
	private function get_payment_page_url( $order, $express = false ) {
		...
		if ( ! empty( $order ) && $order instanceof WC_Order ) {
			...
			$callback_url = home_url() . '?dch-ajax=dch_update_ord';
	...
	public function cancel( $order_id ) {
		...
		if ( ! empty( $order ) &&
			 ...
			 'dintero-checkout' === $order->get_payment_method() ) {
		...
		if ( ! empty( $order ) &&
			...
			if ( array_key_exists( 'status', $transaction ) &&
				...
				if ( array_key_exists( 'status', $response_array ) &&
					...
					WC_AJAX_CH::payment_complete( $order, $transaction_id, $note );
				}
			}
		}
	}
	...
	private function writeContainerScript() {
		echo( "<script type=\"text/javascript\">
	        	var dhpc = document.getElementById('dch_container');
