...
final class WC_Dintero_CH {
	...
	private function init_hooks() {
		...
		add_action( 'dch_after_checkout_form', array( $this, 'init_checkout' ), 50);
		...
		add_action( 'dch_payment_tab', array( $this, 'create_checkout_nav' ));

		//if ( 'no' == $express_enable || ( 'yes' == $express_enable && 'no' == $embed_enable ) ) {
			add_action( 'dch_checkout_billing', array( $this, 'checkout_form_billing' ) );
			add_action( 'dch_checkout_shipping', array( $this, 'checkout_form_shipping' ) );
		//}
		...
		add_action( 'dch_business_customer', array( $this, 'check_dintero_shipping' ));	
    ...
    public function express_checkout_button_shortcode() {
        ...
        if ( WCDCH()->setting()->get('express_enable') !='yes') return;
        ob_start();
    ...
    public function cart_express_checkout_button() {
        ...
        if ( WCDCH()->setting()->get('express_enable') =='yes'){
    ...
    public function maybe_submit_wc_checkout(){
                ...
                function checkIfOrderNowExists(){
                    ...
                    var data = {
                            ...
                            transaction_id : transactionId
                                    ...
                                    var url ='?dch-ajax=check_order_status';
        ...
        $transaction = WCDCH()->checkout()->get_transaction( $transaction_id );
    ...
    public function get_buy_now_button($product_id,$variation_id=null,$sku=null,$disabled=false, $classes='') {
        ...
        $buynow = __('Buy now with','dintero-checkout');
        $title = __('Buy now with dintero', 'dintero-checkout');
...
# Extra classes, if passed IOK 2019-02-26
    ...
    public function cart_express_checkout_button_html() {
        ...
        $imgurl = plugins_url('dintero-checkout/assets/images/dintero-express-btn.png');
        $title = __('Buy now with Dintero!', 'dintero-checkout');
        	...
        	echo( "<script type=\"text/javascript\">
				...
				function dintero_express_checkout(){
        			...
        			var data = {
		            		...
		            		var url = \"".home_url().'?dch-ajax=create_order'."\";
    ...
    public function express_checkout_banner_html() {
        $url = $this->express_checkout_url();
        $url = wp_nonce_url($url,'express','sec');
        $text = __('Skip entering your address and just checkout using', 'dintero-checkout');
        $linktext = __('express checkout','dintero-checkout');
        $logo = plugins_url('dintero-checkout/assets/images/dintero.png');
	...
	public function define_constants() {
		$this->define( 'dch_ABSPATH', dirname( DCH_PLUGIN_FILE ) . '/' );
	}
	...
	public function includes() {
		include_once dch_ABSPATH . 'includes/class-wc-dintero-ch-setting.php';
		include_once dch_ABSPATH . 'includes/class-wc-dintero-ch-ajax.php';
		include_once dch_ABSPATH . 'includes/class-wc-dintero-ch-checkout.php';
		include_once dch_ABSPATH . 'includes/admin/class-wc-dintero-ch-admin-menus.php';
		include_once dch_ABSPATH . 'includes/admin/class-wc-dintero-ch-admin-settings.php';
	}
	...
	public function init_script() {
        ...
        if ( function_exists( 'is_woocommerce' ) ) {
            if ( is_cart() || is_checkout() ) {
        		...
        		$handle = 'dhp-hp';
        		$src = plugin_dir_url(__DIR__) . 'assets/js/dintero_ch.js';
        		$deps = array( 'jquery' );
        		$version = false;
	...
	public function init_footer() {
		echo( '<div class="dch_footer_logo">' . wp_kses_post( WCDCH()->checkout()->get_icon_footer() ) . '</div>' );
	}
	...
	public function cancel_order( $order_id) {
		WCDCH()->checkout()->cancel($order_id);
	}
	...
	public function check_status( $order_id, $previous_status, $current_status) {
		WCDCH()->checkout()->check_status( $order_id, $previous_status, $current_status );
	}
	...
	public function plugin_url() {
		return untrailingslashit( plugins_url( '/', DCH_PLUGIN_FILE ) );
	}
	...
	public function plugin_path() {
		return untrailingslashit( plugin_dir_path( DCH_PLUGIN_FILE ) );
	}

	/**
	 * Get Checkout Class.
	 *
	 * @return WC_Dintero_CH_Checkout
	 */
	public function checkout() {
		return WC_Dintero_CH_Checkout::instance();
	}

	/**
	 * Get Setting Class.
	 *
	 * @return WC_Dintero_CH_Setting
	 */
	public function setting() {
		return WC_Dintero_CH_Setting::instance();
	}
	...
	public function override_template( $template, $template_name ) {
		...
		if ( is_checkout() && WCDCH()->setting()->get('enabled') == 'yes') {
			// Fallback Order Received, used when WooCommerce checkout form submission fails.
			if ( 'checkout/thankyou.php' === $template_name ) {
				if ( isset( $_GET['dch_checkout_error'] ) && 'true' === $_GET['dch_checkout_error'] ) {
					$template = dch_ABSPATH . 'templates/dch-checkout-order-received.php';
				}
			}

			// Don't display template if we have a cart that doesn't needs payment.
			
			if ( apply_filters( 'dch_check_if_needs_payment', true ) ) {
			...
			if ( 'checkout/form-checkout.php' === $template_name ) {
				...
				/*if ( 'yes' == $express_enable ) {
					return $template;
					// return dch_ABSPATH . 'templates/dhp-checkout-noembed-express.php';
					// There SHD be NO Change In Checkout page

				
				}else */
				if ( 'yes' == $embed_enable ) {
					if ( locate_template( 'woocommerce/dch-checkout-embed-express.php' ) ) {
						return locate_template( 'woocommerce/dch-checkout-embed-express.php' );
					} else {
						return dch_ABSPATH . 'templates/dch-checkout-embed-express.php';
					}
				}else {				
					return $template;
				}
			...
			if ( 'checkout/form-pay.php' === $template_name ) {
				return dch_ABSPATH . 'templates/dch-pay.php';
			}			
		}

		// Order detail customer info
		if ( 'order/order-details-customer.php' === $template_name ) {
			return dch_ABSPATH . 'templates/order/order-details-customer.php';
		}
  ...
  //   	if($chosen_shipping_methods[0] != $newMethod[0]){
	...
	public function create_checkout_nav() {
		...
		if ( count( $enabled_gateways ) > 1) {
			...
			foreach ( $enabled_gateways as $gateway ) {
				...
				$rel = 'dintero-checkout' == $id ? 'dhp-embed' : 'dhp-others';
				
				if ( 'dintero-checkout' == $id ) {
					echo( '<div id="' . esc_attr( $id ) . '" rel="' . esc_attr ( $rel ) . '" style="width:' . esc_attr( $tab_w ) . '%;background-image: url(\'' . wp_kses_post( WCDCH()->checkout()->get_icon_tab() ) . '\');"></div>' );
				} else {
	...
	public function check_dintero_shipping( $order ) {
		if ( ! empty( $order ) && $order instanceof WC_Order ) {
			...
			if ( 'dintero-checkout' == $payment_method ) { // && $order->get_transaction_id()
