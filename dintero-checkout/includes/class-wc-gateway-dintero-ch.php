 ...
 * @subpackage dintero-checkout/includes
 */
class WC_Gateway_Dintero_CH extends WC_Payment_Gateway {

	/**
	 * Class constructor.
	 */
	public function __construct() {
		$this->id                 = 'dintero-checkout'; // payment gateway plugin ID
		$this->has_fields         = false;
		$this->method_title       = __( 'Dintero Checkout Express' );
		$this->method_description = __( 'Express Checkout from Dintero.' ); // will be displayed on the options page
	...
	public function process_payment_handler( $order_id ) {
		...
		if ( $order_id && $dintero_order_transaction_id ) {
			...
			$transaction = WCDCH()->checkout()->update_transaction($dintero_order_transaction_id, $order_id);
	...
	private function get_icon_footer() {
		return WCDCH()->checkout()->get_icon_footer();
	}

	private function get_icon_checkout() {
		return WCDCH()->checkout()->get_icon_checkout();
	}
