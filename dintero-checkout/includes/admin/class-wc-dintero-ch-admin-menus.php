...
if ( class_exists( 'WC_Dintero_CH_Admin_Menus', false ) ) {
	return new WC_Dintero_CH_Admin_Menus();
}

/**
 * WC_Admin_Menus Class.
 */
class WC_Dintero_CH_Admin_Menus {
	...
	public function settings_page_init() {
		...
		WC_Dintero_CH_Admin_Settings::get_settings_pages();

		$nonce = wp_create_nonce( 'dhp-nonce' );
			echo( '<input type="hidden" id="_dch_setting_nonce" name="_dch_setting_nonce" value="' . esc_attr( $nonce ) . '" />' );

		// Add any posted messages.
		if ( ! empty( $_GET['wc_error'] ) ) { // WPCS: input var okay, CSRF ok.
			WC_Dintero_CH_Admin_Settings::add_error( wp_kses_post( wp_unslash( $_GET['wc_error'] ) ) ); // WPCS: input var okay, CSRF ok.
		}

		if ( ! empty( $_GET['wc_message'] ) ) { // WPCS: input var okay, CSRF ok.
			WC_Dintero_CH_Admin_Settings::add_message( wp_kses_post( wp_unslash( $_GET['wc_message'] ) ) ); // WPCS: input var okay, CSRF ok.
		}

		//do_action( 'woocommerce_dch_settings_page_init' );
	}
		...
		try {
			if ( isset( $_REQUEST['_dch_setting_nonce'] ) ) {
				...
				if ( ! wp_verify_nonce( $nonce, 'dhp-nonce' ) ) {
					...
					$current_tab     = empty( $_GET['tab'] ) ? 'dintero-checkout' : sanitize_title( wp_unslash( $_GET['tab'] ) ); // WPCS: input var okay, CSRF ok.
					...
					if ( '' !== $current_section && apply_filters( "woocommerce_save_settings_{$current_tab}_{$current_section}", ! empty( $_POST['save'] ) ) ) { // WPCS: input var okay, CSRF ok.
						WC_Dintero_CH_Admin_Settings::save();
	...
	public function settings_page() {
		WC_Dintero_CH_Admin_Settings::output();
	}
}
