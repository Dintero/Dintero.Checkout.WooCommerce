...
if ( ! class_exists( 'WC_Dintero_CH_Admin_Settings', false ) ) :

	/**
	 * WC_Admin_Settings Class.
	 */
	class WC_Dintero_CH_Admin_Settings {
		...
		public static function get_settings_pages() {
			if ( empty( self::$settings ) ) {
				...
				include_once dirname( __FILE__ ) . '/settings/class-wc-dintero-ch-settings-page.php';

				$settings[] = include 'settings/class-wc-dintero-ch-settings-checkout.php';
		...
		public static function save() {
			...
			check_admin_referer( 'woocommerce-dintero-checkout-settings' );
		...
		public static function output() {
			...
			do_action( 'woocommerce_dch_settings_start' );
				...
				switch ( $value['type'] ) {
					...
					case 'tel':
						...
						?><tr valign="top">
							<th scope="row" class="titledesc">
								<label for="<?php echo esc_attr( $value['id'] ); ?>"><?php echo esc_html( $value['title'] ); ?> <?php wp_kses_post( $tooltip_html ); // WPCS: XSS ok. ?></label>
							</th>
							<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
								<input
									...
									<?php wp_kses_post( implode( ' ', $custom_attributes ) ); // WPCS: XSS ok. ?>
									/><?php echo esc_html( $value['suffix'] ); ?> <?php wp_kses_post( $description ); // WPCS: XSS ok. ?>
							</td>
						</tr>
						<?php
						break;
						...
						<tr valign="top">
							...
							<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">&lrm;
								...
								<input
									...
									placeholder="<?php echo esc_attr( $value['placeholder'] ); ?>"
									<?php wp_kses_post( implode( ' ', $custom_attributes ) ); // WPCS: XSS ok. ?>
									/>&lrm; <?php wp_kses_post( $description ); // WPCS: XSS ok. ?>
					...
					case 'textarea':
						...
						<tr valign="top">
							...
							<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
								<?php wp_kses_post( $description ); // WPCS: XSS ok. ?>
						...
						<tr valign="top">
							...
							<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
								<select
									...
									?>
								</select> <?php wp_kses_post( $description ); // WPCS: XSS ok. ?>
							</td>
						</tr>
						<?php
						break;
						...
						?>
							<label for="<?php echo esc_attr( $value['id'] ); ?>">
								<input
									...
									<?php wp_kses_post( implode( ' ', $custom_attributes ) ); // WPCS: XSS ok. ?>
								/> <?php wp_kses_post( $description ); // WPCS: XSS ok. ?>
							</label> <?php wp_kses_post( $tooltip_html ); // WPCS: XSS ok. ?>
						<?php
					...
					case 'image_width':
						...
						<tr valign="top">
							<th scope="row" class="titledesc">
							<label><?php echo esc_html( $value['title'] ); ?> <?php wp_kses_post( $tooltip_html ) . wp_kses_post( $disabled_message ); // WPCS: XSS ok. ?></label>
						</th>
							<td class="forminp image_width_settings">

								<input name="<?php echo esc_attr( $value['id'] ); ?>[width]" <?php wp_kses_post( $disabled_attr ); // WPCS: XSS ok. ?> id="<?php echo esc_attr( $value['id'] ); ?>-width" type="text" size="3" value="<?php echo esc_attr( $width ); ?>" /> &times; <input name="<?php echo esc_attr( $value['id'] ); ?>[height]" <?php wp_kses_post( $disabled_attr ); // WPCS: XSS ok. ?> id="<?php echo esc_attr( $value['id'] ); ?>-height" type="text" size="3" value="<?php echo esc_attr( $height ); ?>" />px

								<label><input name="<?php echo esc_attr( $value['id'] ); ?>[crop]" <?php wp_kses_post( $disabled_attr ); // WPCS: XSS ok. ?> id="<?php echo esc_attr( $value['id'] ); ?>-crop" type="checkbox" value="1" <?php checked( 1, $crop ); ?> /> <?php esc_html_e( 'Hard crop?', 'woocommerce' ); ?></label>
								...
								<select multiple="multiple" name="<?php echo esc_attr( $value['id'] ); ?>[]" style="width:350px" data-placeholder="<?php esc_attr_e( 'Choose countries&hellip;', 'woocommerce' ); ?>" aria-label="<?php esc_attr_e( 'Country', 'woocommerce' ); ?>" class="wc-enhanced-select">
									...
									if ( ! empty( $countries ) ) {
										foreach ( $countries as $key => $val ) {
											echo '<option value="' . esc_attr( $key ) . '"' . wc_selected( $key, $selections ) . '>' . wp_kses_post( $val ) . '</option>'; // WPCS: XSS ok.
										}
									}
									?>
								</select> 
							...
							<input
								...
								<select name="<?php echo esc_attr( $value['id'] ); ?>[unit]" style="width: auto;">
									...
									foreach ( $periods as $value => $label ) {
										echo '<option value="' . esc_attr( $value ) . '"' . selected( $option_value['unit'], $value, false ) . '>' . wp_kses_post( $label ) . '</option>';
									}
									?>
								</select> <?php echo ( $description ) ? wp_kses_post( $description ) : ''; // WPCS: XSS ok. ?>
							</td>
						</tr>
						<?php
						break;
		...
		public static function save_fields( $options, $data = null ) {
			try {
				if ( isset( $_REQUEST['_dch_setting_nonce'] ) ) {
					$nonce = sanitize_text_field( wp_unslash( $_REQUEST['_dch_setting_nonce'] ) );
